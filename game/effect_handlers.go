...
func handleModifyStatEffect(g *GameManager, roomID, _ string, _ *GameStateFull, player *GamePlayer, effect Effect) {
...
func handleDamageEffect(g *GameManager, roomID, _ string, _ *GameStateFull, player *GamePlayer, effect Effect) {
	attrName := effect.Stat
	if attrName == "" {
		attrName = "might"
	}
...
func handleAddStatusEffect(g *GameManager, roomID, _ string, _ *GameStateFull, player *GamePlayer, effect Effect) {
	statusType := effect.Attribute
	if statusType == "" {
		statusType = effect.Message
	}
	if statusType == "" {
		return
	}
